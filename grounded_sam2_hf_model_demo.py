...
parser.add_argument("--text-prompt", default="plants.")
...
for file in os.listdir(INPUT_DIR):
    if file.lower().endswith(".jpg"):
        ...
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"{filename}_groundingdino_annotated_image.jpg"), annotated_frame)

        mask_annotator = sv.MaskAnnotator(color=ColorPalette.from_hex(CUSTOM_COLOR_MAP))
        annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"{filename}_grounded_sam2_annotated_image_with_mask.jpg"), annotated_frame)
